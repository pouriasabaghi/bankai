<x-ui.form.Input name="receives[{{ $loop->index }}][amount]"
    value="{{ $receive->amount ?? '' }}" label="مبلغ" col='4' />

<x-ui.form.Select class="form-control" name='card_id' col='4' label='حساب'>
    @forelse ($cards as $card)
        <x-ui.form.Option
            selected="{{ !empty($card->id) && $card->id == $contract->card_id }}"
...
<x-ui.form.Select class="form-control" name='customer_id' col='4'
    label='مجموعه‌'>
    @forelse ($companies as $company)
        <x-ui.form.Option
            selected="{{ !empty($company->company->id) && $company->id == $company->company->id }}"
