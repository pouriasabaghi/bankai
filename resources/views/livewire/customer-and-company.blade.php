{{-- When $contractCustomerId has value its mean this is edit form --}}
<div class="row position-relative ">
    ...
    <x-ui.form.Select name='customer_id' livewire="wire:model=selectedCustomer" class="form-control _customers-select"
        ...
        @if ($firstTime && empty($contractCustomerId))
            <x-ui.form.Option value="0">انتخاب مشتری</x-ui.form.Option>
        @endif

        @forelse ($customers as $customer)
            <x-ui.form.Option selected="{{ $contractCustomerId == $customer->id }}" value="{{ $customer->id }}">
                {{ $customer->name }}</x-ui.form.Option>
        @empty
    ...
    <x-ui.form.Select name='company_id' livewire="wire:model=selectedCompany" class="form-control _companies-select"
        ...
        @forelse ($companies as $company)
            <x-ui.form.Option selected="{{ $contractCompanyId == $company->id }}" value="{{ $company->id }}">
                {{ $company->name }}
            </x-ui.form.Option>
        @empty
